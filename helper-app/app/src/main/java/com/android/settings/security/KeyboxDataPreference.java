    ...
    public void onBindViewHolder(PreferenceViewHolder holder) {
        ...
        deleteButton.setOnClickListener(v -> {
            Settings.Secure.putString(getContext().getContentResolver(),
                    "keybox_data", null);
            Toast.makeText(getContext(), "XML data cleared",
        ...
        try (InputStream inputStream =
            ...
            if (!validateXml(xml)) {
                Toast.makeText(getContext(),
                        "Invalid XML: missing required data",
                        Toast.LENGTH_SHORT).show();
                return;
            }
            Settings.Secure.putString(getContext().getContentResolver(),
                    "keybox_data", xml);
            Toast.makeText(getContext(), "XML file loaded",
                    Toast.LENGTH_SHORT).show();
            callChangeListener(xml);
        } catch (IOException e) {
            Log.e(TAG, "Failed to read XML file", e);
            Toast.makeText(getContext(), "Failed to read XML",
                    Toast.LENGTH_SHORT).show();
        }
    }
                    ...
                    switch (name) {
                        case "NumberOfKeyboxes":
                            ...
                            if (parser.getEventType() == XmlPullParser.TEXT) {
                                try {
                                    numberOfKeyboxes = Integer.parseInt(
                                            parser.getText().trim());
                            ...
                            break;
                        case "Key":
                            currentAlg = parser.getAttributeValue(
                                    null, "algorithm");
                            ...
                            break;
                        case "PrivateKey": {
                            String format = parser.getAttributeValue(
                                    null, "format");
                            if (!"pem".equalsIgnoreCase(format)) return false;
                            if ("ecdsa".equalsIgnoreCase(currentAlg)) {
